...
# define how many positive and negative reviews are needed to transition, manualy
# for every threshold
R.low <- 0.4
R.high <- 0.7
...
reg1 <- plm(log(number) ~ log(numberLag) + price +
            ...
            log(reviews+1) + score + age + I(age<=7) + day,
                data=panel,
            model="within")
...
reg2 <- plm(log(number) ~ log(numberLag) + price +
              ...
              realPositive + vPositive + ovPositive +
              log(reviews+1) + score + age + I(age<=7) + day + week,
                data=panel,
            model="within")
...
for (id in revs[, ID]){
   sums <- panel[ID==id, .(disc, lam)][, sum(lam), by = disc]
   ...
   HposInv <- solve(Hpos, id.matrix)

   Hneg <- matND/revs[ID==id, rhoNegND] + matD/revs[ID==id, rhoNegD]
   HnegInv <- solve(Hpos, id.matrix)
...
The main parameters of interest in this section are $(\rho_{1i}^+, \rho_{1i}^-)$, which measure the change in the review rate during discounts that is not due to the increase in mere volume of the reviews. The precision with which this parameter could be estimated depends on the number of discount days we observe for game $i$. For some games it is impossible to estimate the parameter precisely, either due to their short presence in the sample, insufficient number of discounts, or both. However, out of `r info[, .N]` games in the sample, `r revs[posSignif == T, .N]` have a value of $\rho_{1i}^+$ that is statistically different from zero. The distributions of $\rho_{1i}^+$ and $\rho_{1i}^-$ for those games are plotted in Figure \ref{likeRatesPlot}.

```{r like-rates, include = F}
p1 <- ggplot(data = revs[, .(x = rhoPosD-rhoPosND)][x>=-0.5& x<=2,]) +
  ggtitle("Discount Contribution to Likes") +
  geom_histogram(mapping = aes(x = x, y = ..density..),
                 color = "black",
                 position = "identity",
                 boundary = 0,
...
p2 <- ggplot(data = revs[, .(x = rhoNegD-rhoNegND)][x>=-0.5& x<=2,]) +
  ggtitle("Discount Contribution to Dislikes") +
  geom_histogram(mapping = aes(x = x, y = ..density..),
                 color = "black",
                 position = "identity",
                 boundary = 0,
...
panel <- merge(panel, revs[, .(ID, rho1 = rhoPosD - rhoPosND, rhoPosD, rhoNegD)])
...
star.out<-stargazer(reg1, reg2,
                    ...
                    # column.labels=c("OLS", "Interaction", "FE", "FE: no $\\rho_{1i}^+$"),
                    model.numbers = F,
