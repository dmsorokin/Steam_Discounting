...
reg1 <- lm(I(discount>0) ~  tToTransitNeg + tToTransitPos + 
              ...
              log(reviews) + age + poly(tWODisc, degree = 2) + day + week, data = transPanel)
cov<-vcovHC(reg1)
se1 <- sqrt(diag(cov))
...
star.out.transitions <- stargazer(reg1,
          ...
          covariate.labels = c("Days to Transition", "Days After Transition", "No Score", 
                               ...
                               "Ov. Positive", "Log Reviews", "Age", "Const"),
          model.numbers = F,
...
reg3 <- plm(I(discount>0) ~ pos.treatment + neg.treatment +
               log(needPosDays) + log(needNegDays) + poly(tWODisc, degree = 2) + log(reviews+1) +
...
star.out <- stargazer(reg1, reg2, reg3,
             ...
             covariate.labels = c("Close to Pos. Transition", "Close to Neg. Transition",
                                  ...
                                  "Log Reviews", "Mostly Positive", "Positive",
                                  "Very Positive", "Age", "Age $\\le 14$"),
             model.numbers = F,
             omit = c("day", "week", "tWODisc"),
             omit.stat=c("f","adj.rsq","ser"),
             no.space=T)
...
\log A_{it} = \tilde\lambda_i + \tilde\psi_i \log A_{it-1} + x'_{it}\tilde \beta + u_{it}
$$
\noindent To check whether the omitted covariates could be playing a crucial role, I will report the results of estimaing (\ref{logReg}) both with the ideal set of covariates, and with the covariates used to estimate (\ref{arrivalReg}). The results are presented in Table \ref{demandEstimates}.
...
```{r results='asis'}
reg1 <- plm(log(number) ~ log(numberLag) + log(price) + noScore +
              negative + mPositive + positive + log(reviews+1) + 
              age + young + discSeason + day,
                 data=panel[price!=0,],
            model="within")

reg2 <- plm(log(number) ~ log(numberLag) + log(price) + noScore + 
              negative + mPositive + realPositive + vPositive + 
              ovPositive + log(reviews+1) + age + young + 
              day + week,
                data=panel[price!=0,],
            model="within")
...
table <- texreg(res,
                ...
                digits = 3,
                custom.model.names = c("NLLS", "Reg", "Full Reg"),
...
cat(star_insert_row(star.out.transitions, c("Time Effects & Weekdays, Week \\\\",
                                "Game Effects & $\\times$ \\\\",
                                "Polynomial($t$ W/O Discount) & $d = 2$ \\\\"), insert.after = c(34,34,34)))
```
...
reg1 <- plm(new.reviews ~ discNew + discNewLag + discNewLag2 + discNewLag3 + discNewLag4 +
                          ...
                          negative + mPositive + positive + age + young + day + week,
...
star.out <- stargazer(reg1,
             ...
             model.numbers = F,
             omit = c("day","week","negative","mPositive","positive","age", "young"),
             omit.stat=c("f","adj.rsq","ser"),
             no.space=T)
