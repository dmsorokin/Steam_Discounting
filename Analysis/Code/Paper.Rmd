...
tr <- createTexreg(coef = theta1,
                   coef.names = c("Prob. of Like", "Discount on Like", 
                                  "Prob. of Dislike", "Discount on Dislike"),
                   se = sqrt(diag(omega1)),
                   pvalues = pvalues,
                   gof.names = gofnames,
                   gof = c(dim(revs)[1]))
...
tr <- createTexreg(coef = theta2,
                   coef.names = c("Prob. of Like", "Discount on Like", 
                                  "Young on Like", "Old on Like", 
                                  "Prob. of Dislike", "Discount on Dislike",
                                  "Young on Dislike", "Old on Dislike"),
                   se = sqrt(diag(omega2)),
                   pvalues = pvalues,
                   gof.names = gofnames,
                   gof = c(dim(revs)[1]))
...
Setting aside the aforementioned issues, my results suggest that a typical user on Steam leaves a positive review with a probability around `r round(theta1[1]*100)`%, and leaves a negative review with a probability around `r theta1[3]*100`%. Discounts appear to increase both probabilities, implying that users seem to be more likely to leave a review during a discount. I formally test this hypothesis, stated in (\ref{hyp1}), and I strongly reject the null that users are less likely to leave reviews when buying on a discount in favor of the alternative that they are more likely to leave reviews when buying on a discount (the values of the $t$-stats are `r round(t1.1[1],2)` and `r round(t1.2[1],2)` in the two specifications). My second hypothesis, stated in (\ref{hyp2}), is that reviews are less positive during a discount than they are outside the discount. I fail to reject the null in favor of the alternative that reviews left during a price promotion are more positive (the $p$-values are `r round(1-pnorm(t2.1[1]),2)` and `r round(1-pnorm(t2.2[1]),2)`). As we can see, there is no disagreement across specifications on both hypotheses, giving me confidence that this result is robust. As for the age controls,  I find that users are more likely to leave reviews in the first two weeks, but, somewhat surprisingly, do not lose steam as the game becomes older. Both findings are in accordance with the patterns or reviewing dynamics depicted in Figure \ref{reviewDescriptivePlots}.
...
Now that I have carried out the analysis of the average reviewing propensity and valence, I turn attention to heterogeneity between firm. In the identification section I showed how one can estimate the like and dislike rates on the product level. The theory of discounting close to review transitions developed in this paper suggests that products whose customers leave better reviews during a price promotions should be more likely to discount when they are close to a transition, regardless of whether it is a positive or a negative one. It is convenient to introduce notation for the difference between the expected review scores on and off a discount for the games in my sample:

\begin{equation}
\Delta score_{i} := \frac{\rho^+_{0i} + \rho^+_{1i}}{\rho^+_{0i} + \rho^+_{1i} + \rho^-_{0i} + \rho^-_{1i}} - \frac{\rho^+_{0i}}{\rho^+_{0i} + \rho^-_{0i}} 
\end{equation}

Figure \ref{scoreDifPlot} depicts the distribution of $\widehat{\Delta score_{i}}$, the estimated differences in the expected review scores on and off a discount for the games in my sample. A couple of paragraphs ago I have shown that the average difference is not statistically distinguishable from zero, and the plot complies with that finding. Estimates of $\Delta score_{i}$ vary in their precision from game to game, as games that spent less little time in the sample and/or on discounts would naturally have noisier estimates. However, the distribution appears to be well-behaved.


With these estimates at hand, I can now revisit Model (\ref{potTransitionsReg}), the regression I used to establish the effect of proximity to a potential transition on the discounting probability. I modify the equation by including the interaction between the 
treatment variables of interest (proximities to potential transitions) and $\Delta score_{i}$, leading to
\begin{equation}\label{potTransitionsHeteroReg}
disc_{it} = (\beta^+_0 + \Delta score_{i}\beta^+_1)T_{it}^+ +(\beta^-_0 + \Delta score_{i}\beta^-_1)T_{it}^- + \Delta score_{i} + X_{it}\beta + f_i + \tau_t + \varepsilon_{it}
\end{equation}

\noindent As $\Delta score_{i}$ does not change over time, it can not be estimated by the within-estimator, but the interaction term could still be included. My theory implies that $\beta^+_1$ and $\beta^-_1$ should be positive. I estimate (\ref{potTransitionsHeteroReg}) by OLS (without the fixed effect term $f_i$) and by the within-estimator. The results are presented in Table \ref{scoreDifTransDiscReg} (the controls are omitted for brevity). 
...
The least squares estimates in column (OLS) are mainly presented to check the correlations in the data. Better sentiment of reviews left during a discount is not related to the frequency of discount on its own, but products close to a positive transition are more likely to discount if their customers are more lenient when buying during a price promotion. This effect is not present in the preferred specification (\ref{potTransitionsHeteroReg}), which is estimated in the second column. Quite interestingly, this specification finds $\beta^-_0 < 0$, while my earlier results, essentially, failed to find any effect of a proximity to a negative transition on discounting behavior. This finding makes a lot of sense. Notice that $\beta^-_0$ measures the effect of the aforementioned proximity on discounting probability conditional on $\Delta score_{i} = 0$. For a game that is close to downgrading its review score and which does not receive better reviews on a discount, initiating a price promotion is a gamble with no clear upside and a significant downside. This finding complies perfectly with the prediction of the stylized model of discounting behavior presented in section \ref{stylizedModel}.
