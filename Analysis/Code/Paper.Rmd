...
```{r}
transitions <- panel[, .(t, age,
                    noscore=c(NA, diff(noScore)),
                    neg=c(NA, diff(negative)),
                    mix=c(NA, diff(mixed)),
                    mpos=c(NA, diff(mPositive)),
                    pos=c(NA, diff(realPositive)),
                    vPos=c(NA, diff(vPositive)),
                    ovPos=c(NA, diff(ovPositive)),
                    noScore, negative, mixed, mPositive, 
                    realPositive, vPositive, ovPositive), by=ID]
transitions <- transitions[noscore==-1 | neg ==-1 | mix ==-1 | 
                           mpos == -1 | pos == -1 | 
                           vPos == -1 | ovPos == -1, ]
transitions[noscore==-1, origin := "No Score"]
...
# based on the estimation of demand, classify any transition from No Score as good
transitions[noscore==-1 | neg==-1 | (mix==-1 & neg!=1) | (mpos == -1 & mix != 1 & neg!= 1) | (pos==-1 & (vPos==1 | ovPos==1)) | (vPos==-1 & ovPos == 1), 
            goodTr := T]
...
kable(
   cbind(
      ...
      linesep = "",
      booktabs = T) %>%
add_header_above(c("Probabilities" = 7, " ","Counts" = 7)) %>%
kable_styling(latex_options = c("scale_down"))
```

```{r}
noscore.ids <- transitions[noscore==-1 & age >=10, ID]
```
...
star.out.transitions <- stargazer(reg1,
          ...
          covariate.labels = c("Days to Transition", "Days After Transition",
                               "No Score", "Negative", "Mostly Positive", "Positive", "Very Positive",
...
```{r}
noscore.ids <- transitions[noscore==-1 & age >=10, ID]
revs[ID %in% noscore.ids, summary(rhoPosND)]
```
