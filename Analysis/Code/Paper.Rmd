...
The second imprortant set of variables are the price variables. 
...
panel[, disc:=c(shift((discount > 0))),
      by = ID][is.na(disc), disc := F]
revs <- panel[, .(t, disc, lambda, l = c(pReviews[1], diff(pReviews)), d = c(nReviews[1], diff(nReviews))), by=ID]
revs <- revs[, .(rhoPosD = sum(disc*l) / sum(disc*lambda),
                 rhoPosND = sum((1-disc)*l) / sum((1-disc)*lambda),
                 rhoNegD = sum(disc*d) / sum(disc*lambda),
                 rhoNegND = sum((1-disc)*d) / sum((1-disc)*lambda)),
              by = ID]
revs <- revs[!is.na(rhoPosD)]

id.matrix <- matrix(c(1,0,0,1), nrow=2)
for (id in revs[, ID]){
   sums <- panel[ID==id, .(disc, lambda)][, sum(lambda), by = disc]
...
```{r like-rates, fig.width = 8, fig.height = 6, include = F}
p1 <- ggplot(data = revs[posSignif==T, .(x = rhoPosD-rhoPosND)][x>=-0.5& x<=2,]) +
...
p2 <- ggplot(data = revs[posSignif==T, .(x = rhoNegD-rhoNegND)][x>=-0.5& x<=2,]) +
...
```{r fig.cap='\\label{likeRatesPlot} Distributions of $\\rho_{1i}^+$ and $\\rho_{1i}^-$', fig.width=9}
ggarrange(p1, p2, nrow=1, align = "h")
```
